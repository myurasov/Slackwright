...
def _render_text(text: str, users: dict[str, dict[str, Any]],
                 channels: dict[str, dict[str, Any]]) -> str:
    ...
    for m in pattern.finditer(text):
        ...
        if m.group("u"):
            ...
            label = m.group("ulabel") or (
                (users.get(uid) or {}).get("name")
                or (users.get(uid) or {}).get("real_name")
                or uid
            )
...
def render_report(out_dir: Path, *, target: Path | None = None,
    ...
    page_title = title or (
        f"Slackwright report — {index.get('plan') if index else out_dir.name}"
    )
...
def _render_channel_section(cid: str, msgs: list[_Message],
                            channels: dict[str, dict[str, Any]],
                            users: dict[str, dict[str, Any]],
                            sa_user_id: str | None) -> str:
...
def _render_message(m: _Message, users: dict[str, dict[str, Any]],
                    sa_user_id: str | None, *, is_reply: bool) -> str:
    ...
    permalink = (
        ...
        if m.permalink else html.escape(when)
    )
...
def _author_block(m: _Message, users: dict[str, dict[str, Any]],
                  sa_user_id: str | None) -> str:
...
def _render_files(files: list[dict[str, Any]], raw: dict[str, Any]) -> str:
    ...
    for f in files:
        ...
        items.append(
            f'<span class="file">📎 <a href="{rel}">{html.escape(str(name))}</a></span>'
        )
    if not items:
        return ""
...
def _render_reactions(reactions: list[dict[str, Any]]) -> str:
    ...
    for r in reactions:
        ...
        items.append(
            f'<span class="react">:{html.escape(str(name))}: {int(count)}</span>'
        )
...
def _format_timestamp(ts: float) -> str:
    try:
        return dt.datetime.fromtimestamp(ts, tz=dt.timezone.utc) \
            .astimezone().strftime("%a %b %d %Y, %H:%M")
    except (TypeError, ValueError, OSError):
        return str(ts)
